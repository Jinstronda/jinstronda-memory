...
export const log = {
	info(msg: string, ...args: unknown[]): void {
		_backend.info(`mem0: ${msg}`, ...args)
	},

	warn(msg: string, ...args: unknown[]): void {
		_backend.warn(`mem0: ${msg}`, ...args)
	},

	error(msg: string, err?: unknown): void {
		const detail = err instanceof Error ? err.message : err ? String(err) : ""
		_backend.error(`mem0: ${msg}${detail ? ` - ${detail}` : ""}`)
	},

	debug(msg: string, ...args: unknown[]): void {
		if (!_debug) return
		const fn = _backend.debug ?? _backend.info
		fn(`mem0 [debug]: ${msg}`, ...args)
	},

	debugRequest(method: string, params: Record<string, unknown>): void {
		if (!_debug) return
		const fn = _backend.debug ?? _backend.info
		fn(`mem0 [debug] > ${method}`, JSON.stringify(params, null, 2))
	},

	debugResponse(method: string, data: unknown): void {
		if (!_debug) return
		const fn = _backend.debug ?? _backend.info
		fn(`mem0 [debug] < ${method}`, JSON.stringify(data, null, 2))
	},
}
