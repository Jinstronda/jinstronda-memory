...
### v1.6 - Two-Layer Search + Query Rewriting + User Profiles

Three additive features on top of the v1.2 pipeline. Nothing from v1.2 was removed. Hybrid search, entity graph, reranker, counting decomposition, and all prompt engineering remain unchanged.
...
Sessions
  |
  v
LLM Extraction (GPT-5-mini, batches of 10, global cap 300)
  |
  v
Parse extraction output -> entities, relationships, memoriesText
  |
  +---> Entity Graph (in-memory adjacency list or Postgres)          [UNCHANGED]
  |       add entities + relationships per session
  |
  +---> Chunking (1600 chars, 320 overlap, sentence-boundary)        [UNCHANGED]
  |       -> Embed chunks (text-embedding-3-large, 3072d, batch 100)
  |       -> Store in HybridSearchEngine or pgvector
  |
  +---> Atomic Facts (split memoriesText into individual lines)      [NEW]
  |       -> Embed each fact (same model, same batching)
  |       -> Store in InMemoryFactStore or rag_facts table
  |
  +---> User Profile (GPT-5-mini extracts biographical facts)        [NEW]
          -> Merge with existing profile (word overlap 0.6 dedup)
          -> Cache to disk or keep in memory
```
...
Query
  |
  v
Query Rewrite (GPT-5-mini, expand synonyms/terms)                   [NEW]
  |
  v
Embed rewritten query (text-embedding-3-large, 3072d)
  |
  v
Search Atomic Facts (pure vector cosine, top 30)                    [NEW]
  -> collect matched session IDs for boosting
  |
  v
Hybrid Search (BM25 * 0.3 + Vector * 0.7, overfetch 40)            [UNCHANGED]
  -> boost fact-matched sessions (+0.1 to hybrid score)             [NEW]
  |
  v
Counting query? -> Decompose (GPT-5-mini) + merge sub-queries       [UNCHANGED]
  |
  v
Rerank (GPT-5-mini, query-type-aware, 7 types, top 20)             [UNCHANGED]
  |
  v
Entity Graph (2-hop BFS, max 10 entities, 20 relationships)         [UNCHANGED]
  -> inject as <knowledge_graph> XML
  |
  v
User Profile -> inject as <user_profile> XML                        [NEW]
  |
  v
Answer (GPT-5, reasoningEffort: medium, chain-of-thought)           [UNCHANGED]
  |
  v
Judge (GPT-4o, category-specific prompts)                           [UNCHANGED]
```

#### New Modules

**Atomic Facts (`facts.ts`, 102 lines)**

Memories from extraction are split into individual lines (atomic facts). Each fact gets its own embedding. During search, facts are searched first (pure vector cosine, top 30). Sessions matched by facts get a +0.1 score boost on their chunks, so the reranker sees the most relevant content.

Why two layers: atomic facts are clean, self-contained statements that embed precisely. Chunks have surrounding context that LLMs need for reasoning. Search the facts for precision, inject the chunks for context.

Storage:
- In-memory: `InMemoryFactStore` class, serialized to `data/cache/rag/{containerTag}/facts.json`
- Postgres: `rag_facts` table with HNSW vector index

Constants: `FACT_SEARCH_LIMIT = 30`, `FACT_SESSION_BOOST = 0.1`

**Query Rewriting (`rewrite.ts`, 36 lines)**

Every search query is rewritten by GPT-5-mini before embedding. The rewrite expands the query with synonyms, related terms, and key concepts while preserving proper nouns and dates. Falls back to original query on failure or if output exceeds 500 chars.

**User Profiles (`profile.ts`, 102 lines)**

During ingestion, GPT-5-mini extracts static biographical facts (name, location, occupation, relationships, hobbies) into a `UserProfile`. Profile facts persist across sessions and are injected into every search result as `<user_profile>` context. Common facts (user's name, where they live) don't consume search result slots.

Profile merging uses word overlap threshold (0.6) to detect and replace stale facts. Cached to `data/cache/rag/{containerTag}/profile.json`.

#### File Map

```
src/providers/rag/
  index.ts        770 lines   Main provider: ingest, search, caching
  search.ts                   HybridSearchEngine (BM25 + vector, in-memory)
  graph.ts                    EntityGraph (adjacency list, BFS traversal)
  reranker.ts                 LLM reranker (GPT-5-mini, 7 query types)
  decompose.ts                Counting query detection + decomposition
  extraction.ts               (in src/prompts/) Memory extraction prompt
  pg.ts                       PostgreSQL + pgvector backend
  facts.ts                    Atomic fact store (in-memory + serialization)
  rewrite.ts                  Query rewriting (GPT-5-mini)
  profile.ts                  User profile extraction + merging
  prompts.ts                  Answer generation prompt builder
  lock.ts                     Read/write container locking
```

## Planned Improvements

1. **Memory versioning**: Mem0-style ADD/MERGE/INVALIDATE operations. When new info contradicts old, mark old as invalidated. Dedup on entity+attribute.
2. **Contextual Retrieval prepending**: Anthropic technique. Prepend document-level context to each chunk before embedding. Reduces chunk ambiguity.
3. **Time-aware query expansion**: for temporal questions, expand query with date ranges and temporal keywords. "What did I do last Tuesday" becomes the query plus the resolved date.
4. **Cross-encoder reranker**: replace LLM reranker with a dedicated cross-encoder model (e.g., bge-reranker-v2-m3). Faster, cheaper, potentially more consistent scores.
5. **Adaptive K**: dynamically adjust retrieval K based on query type. Counting questions get K=40, simple factual gets K=10.
6. **Memory decay**: reduce relevance of older memories over time. Recent conversations should be weighted higher unless the question specifically asks about the past.
