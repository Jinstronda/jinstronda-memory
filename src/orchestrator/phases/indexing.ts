...
class IndexingProgressTracker {
    private progressByQuestion: Map<string, { completed: number; failed: number; total: number }> = new Map()
    ...
    display(): void {
        ...
        if (displayStr !== this.lastDisplayed) {
            ...
            process.stdout.write(`\r\x1b[36m[${bar}]\x1b[0m ${percent}% Indexing: ${agg.completed}/${agg.total} episodes${failedStr}`)
        }
    }
    ...
    finish(): void {
        const agg = this.getAggregated()
        const failedStr = agg.failed > 0 ? ` (${agg.failed} failed)` : ""
        process.stdout.write(`\r\x1b[36m[${"█".repeat(20)}]\x1b[0m 100% Indexing: ${agg.completed}/${agg.total} episodes${failedStr}\n`)
    }
...
export async function runIndexingPhase(
    ...
    logger.info(`Awaiting indexing for ${toIndex.length} questions, ${totalEpisodes} episodes (concurrency: ${concurrency})...`)
    ...
    await ConcurrentExecutor.execute(
        ...
        async ({ item: question }) => {
            ...
            try {
                let lastProgress: IndexingProgress = { completedIds: [], failedIds: [], total: episodeCount }
